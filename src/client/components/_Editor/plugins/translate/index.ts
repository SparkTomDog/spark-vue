class TranslateTool {
    ...
    tooltipSheet() {
        ...
        return sheets.filter((sheet) => sheet.ownerNode?.id === 'Translate');
    }
     ...
     */
    createTooltip(tooltipValue: any, spanTooltip = this.spanTooltip) {
        if (this.spanTooltip) {
    ...
    setBackgroundColor(spanTooltip: null) {
    ...
    setUnderlineDecoration(spanTooltip: null) {
        const tooltip = spanTooltip;
        if (this.underline) {
    ...
    wrap(range: { extractContents: () => any; insertNode: (arg0: null) => void; }) {
     ...
     * @param {object} range is an object with info about the selected text
     */
    unwrap(range: { extractContents: () => any; insertNode: (arg0: any) => void; }) {
    ...
    showActions() {
        ...
        this.api.listeners.on(this.tooltipInput, 'keydown', (e: { key: string; }) => {
            if (e.key === 'Enter') {
    ...
    */
    static get sanitize() {
        return {
            span: (e: { classList: { remove: (arg0: string, arg1: string) => void; }; }) => {
